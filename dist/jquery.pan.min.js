...
var finalSet=$(this).filter(function(){if("IMG"!=this.tagName)return!0;var nW=this.naturalWidth||0,nH=this.naturalHeight||0,w=$(this).outerWidth(),h=$(this).outerHeight();return w<nW||h<nH||void 0});function panInit(event){event.preventDefault();var panImg=$(".panWrapper img.i"),panWrapper=$(".panWrapper"),w=parseInt(panImg.css("width")),h=parseInt(panImg.css("height")),ml=(parseInt(panImg.css("left")),parseInt(panImg.css("top")),0-(w-$(panWrapper).width())),mt=0-(h-$(panWrapper).height()),scrollOffset=window.pageYOffset||document.documentElement.scrollTop,nl=parseInt(ml*parseInt(event.pageX||(event.touches?event.touches[0].pageX:0))/parseInt($(panWrapper).width())),nt=parseInt(mt*parseInt(event.pageY||(event.touches?event.touches[0].pageY:0)-scrollOffset)/parseInt($(panWrapper).height()));parseInt($(panWrapper).width())>w&&parseInt($(panWrapper).height())>h?(panImg.css("left",(parseInt($(panWrapper).width())-w)/2),panImg.css("top",(parseInt($(panWrapper).height())-h)/2)):parseInt($(panWrapper).width())>w?(panImg.css("left",(parseInt($(panWrapper).width())-w)/2),panImg.css("top",nt)):parseInt($(panWrapper).height())>h?(panImg.css("left",nl),panImg.css("top",(parseInt($(panWrapper).height())-h)/2)):(panImg.css("left",nl),panImg.css("top",nt))}return finalSet.css("cursor","zoom-in"),finalSet.click(function(e){var t=$(this),big=t.attr("data-big");
